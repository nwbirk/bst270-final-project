...
To proceed with the analysis, we will first need to load any necessary packages and import the dataset from online. If the dataset is available locally, this option will instead be used to import the dataset. Note that the package `renv` is used to create and environment that saves the exact versions of each package used in the analysis.
...
Some challenges to reproducibility include the fact that more additional parties are included in the dataset than are reported in the paper. This is somewhat addressed in the figure captions, but it was not immediately clear to me where categories such as "Bipartisan, but mostly Republican" should be included or excluded from the Republican total amounts (and same for Democrat). Further, a slight challenge in reproducing Table 2 was in the encoding of the Leagues as a string that included all leagues represented. While this could be helpful in some contexts, it could have been easier for the coding to have a series of 6 binary variables that instead specify whether the corresponding donor owns a team in a given league. Additionally, I did not find it immediately clear whether the "Total" column in Table 2 corresponded to the total across all parties, or just the sum of Republican and Democrat amounts (though this was not difficult to check through a 'trial and error' appoach).